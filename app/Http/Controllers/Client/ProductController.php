...
class ProductController extends Controller
{
    ...
    {
        $data['list'] = Product::where('status',1)->orderBy('id','DESC')->select('id','category','name','discount','price','images','slug','cate_slug','type_slug','species')
        ->paginate(12);
    ...
    public function allListCate($danhmuc)
    {
        ...
        ->orderBy('id','DESC')
        ->select('id','category','name','discount','price','images','slug','cate_slug','type_slug','description','species')
        ->paginate(12);
        ...
        $data['typeList'] = TypeProduct::where('cate_id',$data['cateno']->id)->get(['name','id','cate_slug','slug']);
        return view('product.list',$data);
    }
    public function allListType($danhmuc,$loaidanhmuc){
        $data['list'] = Product::where(['status'=>1,'cate_slug'=>$danhmuc,'type_slug'=>$loaidanhmuc])
            ...
            ->paginate(12);
        $data['pronew'] = Product::where('status',1)->orderBy('id','DESC')->select('id','category','name','discount','price','images','slug','cate_slug','type_slug','species')
        ->paginate(5);
        $data['type'] = TypeProduct::where('slug',$loaidanhmuc)->first(['id','name','cate_id','content','slug']);
        $cate_id = $data['type']->cate_id;
        $data['cateno'] = Category::where('slug',$danhmuc)->first(['id','name','avatar','content','slug']);
        $data['typeTwoList'] = TypeProductTwo::where('type_id',$data['type']->id)->get(['name','id','cate_slug','slug']);
    ...
    public function detail_product($cate,$type,$id)
    {
        ...
        ])->where('slug',$id)->first(['id','name','images','type_cate','category','sku','species','price','content','size','description','slug','origin']);

...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        view()->composer('*', function ($view) 
        {
            ...
            $servicehome = Services::where(['status'=>1])->get();
            $categoryhome = Category::with([
                'typeCate' => function ($query) {
                    $query->with(['products','typetwo'])->where('status',1)->orderBy('id','DESC')->select('cate_id','id', 'name','avatar','slug','cate_slug'); 
                }
            ...
            $bannerAds = BannerAds::where('status',1)->first(['name','image','id','content']);
